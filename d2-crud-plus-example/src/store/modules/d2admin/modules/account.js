...
export default {
  ...
  actions: {
    ...
    async login({ dispatch }, userInfo) {
      ...
      let res = await api.SYS_USER_LOGIN({ accountName: userInfo.username, password: userInfo.password })
      res = res.data
      ...
      util.cookies.set('token', res.loginToken)

      let res1 = await api.SYS_USER_INFO()
      res1 = res1.data
      console.log(res1)
      // 设置 vuex 用户信息
      await dispatch('d2admin/user/set', {
        id: res1.id,
        username: res1.accountName,
        name: res1.realName
      }, { root: true })
      // 用户登录后从持久化数据加载一系列的设置
      await dispatch('load')
